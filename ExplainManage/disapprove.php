...
$sql="UPDATE Video SET vid_status=2,exa_id='$uid',exa_time='$time' WHERE vid_id=$vid";
$res=$conn->query($sql);

if ($res) {
	ini_set('error_log',"$filePath");
    error_log($username." 审核讲解"." ip地址:".$_SERVER['REMOTE_ADDR']."审核讲解:".$vid_name."不通过 ".date("Y-m-d H:i:s"));
