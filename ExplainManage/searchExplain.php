...
$array=array();
if (empty($content)) {
    echo '<script>alert("请输入搜索内容！");location="explain.php"</script>';
}else{
    $sql="select * from Video where $op like '%$content%'";
    $res=$conn->query($sql);
    if ($res) {
        ...
        // var_dump($array);
        if ($array==null) {
            echo '<script>alert("没有找到相关内容(╥﹏╥)o");location="explain.php"</script>';
        }
    }else{
        echo '<script>alert("查询失败(╥﹏╥)o");location="explain.php"</script>';
    }
}

$pageSize = 3;//每一页面内容大小
$res="select * from Video where $op like '%$content%'";
$sql=$conn->query($res);
