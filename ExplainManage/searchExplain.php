...
if (empty($content)) {
    ...
    $sql="select * from comment where com_id like '%$content%'";
    $res=$conn->query($sql);
    if ($res) {
                ...
                <li class="navHeader">
                    <div class="headerBox">
                        <a href="/jk-museum/ExplainManage/explain.php" class="header">
                            <span style="color: yellow">讲解管理</span>
                        </a>
                    </div>
                </li>
                <li class="navHeader">
            ...
            <form class="bs-example bs-example-form" role="form" id="search" action="searchExplain.php" method="post">
                <div class="col-lg-6" id="searchBorder">
                    <div class="input-group" style="margin-top:10px;margin-left:3px">
                        <input type="text" style="margin-top:10px" class="form-control" placeholder="请输入需要搜索的内容" name="content">
                ...
                <a type="submit" class='btn' style="margin-left: 600px;margin-top: 20px;background-color: green" href=addExplain.php><span style="color: whitesmoke">添加新讲解</span></a>
            </form>
        </div>
        <div class="tableStyle"  style="width:1585px;height:850px;overflow:auto">
            <style>
                .style_500px{width:500px;vertical-align:middle}
                .style_150px{width:150px;vertical-align:middle}
                .style_200px{width:200px;vertical-align:middle}
            ...
            <table class="table table-hover table-bordered" style="margin-left:18px;margin-top:20px;width:1550px" id="showMag">
                ...
                if (1) {
                    foreach($array as $v){
                        ?>
                        <tr>
                            <td class="style_150px"><?=$v["com_id"]?></td>
                            <td class="style_150px"><?=$v["mus_id"]?></td>
                            <td class="style_100px"><?=$v["com_grade"]?></td>
                            <td class="style_150px"><?=$v["user_id"]?></td>
                            <td class="style_150px"><?=$v["mus_name"]?></td>
                            <td class="style_500px"><textarea type="text" style="border:white;width:475px;height:100px;overflow:auto;background-color:whitesmoke;resize:none" readonly="readonly"><?=$v["com_info"]?></textarea></td>
                            <td class="style_200px"><?=$v["com_time"]?></td>
                            <td><a type='submit' class='btn' style="background-color: red;margin-top: 13px" href=deleteExplain.php?cid=<?=$v["com_id"]?>><span style="color: whitesmoke">删除</span></a>
                                <br>
                                <a type='submit' class='btn' style="background-color: darkorange;margin-top: 10px" href=modifyExplain.php?cid=<?=$v["com_id"]?>><span style="color: whitesmoke">修改</span></a>
                            </td>
                        </tr>
                        <?php
                    }}
