...
if (empty($content)) {
    ...
    $sql="select * from Comment where $choice like '%$content%'";
    $res=$conn->query($sql);
    if ($res) {
        ...
        <div class="content">
            ...
            <table class="table table-hover table-bordered" id="showMag">
                ...
                <tr>
                    ...
                    <td><?=$v["user_id"]?></td>
                    <td><?=$v["mus_name"]?></td>
                    <td><textarea type="text" style="border:white;width:490px;height:100px;overflow:auto;background-color:whitesmoke;resize:none" readonly="readonly"><?=$v["com_info"]?></textarea></td>
                    <td><?=$v["com_time"]?></td>
                        <?php
                        }}
                        ?>
