...
if ($flag==1) {
	...
	if ($num) {//存在该用户
		...
		if ($res) {
            ...
            error_log($username." 修改用户信息"." ip地址:".$_SERVER['REMOTE_ADDR']."修改用户:".$user_name."成功 ".date("Y-m-d H:i:s"));
			 echo '<script>alert("修改成功！");window.location="user.php"</script>';
		}else{
			ini_set('error_log',"$filePath");
            error_log($username." 修改用户信息"." ip地址:".$_SERVER['REMOTE_ADDR']."修改用户:".$user_name."失败 ".date("Y-m-d H:i:s"));
