...
//分页
$pageSize = 5;//每一页面内容大小
$res="select * from Collection where $op like '%$content%'";
$sql=$conn->query($res);
...
$array=array();
if (empty($content)) {
    echo '<script>alert("请输入搜索内容！");location="collection.php"</script>';
}else{
    $sql="select * from Collection where $op like '%$content%' limit $pageSql,$pageSize";
    $res=$conn->query($sql);
    if ($res) {
        ...
        // var_dump($array);
        if ($array==null) {
            echo '<script>alert("没有找到相关内容(╥﹏╥)o");location="collection.php"</script>';
        }
    }else{
        echo '<script>alert("查询失败(╥﹏╥)o");location="collection.php"</script>';
    }
}
?>
<!DOCTYPE html>
    ...
    <div class="topWrapper">
        ...
        <div class="content">
            ...
            <div class="searchContent">
                <form class="bs-example bs-example-form" role="form" id="search" action="searchCollection.php" method="post">
                    <div class="col-lg-12" id="searchBorder">
                        <select class="form-control" name="op" id="choiceBorder">
                            <option value="col_name">藏品名称</option>
                            <option value="col_id">藏品编号</option>
                            <option value="mus_id">博物馆编号</option>
                            <option value="col_info">藏品简介</option>
                ...
                <a href="addCollection.php" class="btn" style="background-color: green;color: whitesmoke">添加藏品</a>
            </div>
                    ...
                    if (1) {
                        foreach($array as $v){
                            ...
                            <tr>
                                ...
                                <td><?=$v["mus_name"]?></td>
                                <td><?=$v["col_era"]?></td>
                                <td><textarea style="width:80%;border:white;height:100px;overflow:auto;background-color:white;resize:none" readonly="readonly"><?=$v["col_info"]?></textarea></td>
                                <td><a target="_blank" href=<?=$v["col_picture"]?>><img width="130px" height="100px"  src="<?=$v["col_picture"]?>"/></a></td>
                                <td><a type='submit' class='btn' style="background-color: red;margin-top: 13px" href=deleteCollection.php?cid=<?=$v["col_id"]?>><span style="color: whitesmoke">删除</span></a>
                                    <br>
                                    <a type='submit' class='btn' style="background-color:deepskyblue;margin-top: 10px" href=modifyCollection.php?cid=<?=$v["col_id"]?>><span style="color: whitesmoke">修改</span></a>
                                </td>
                            </tr>
                            <?php
                        }}
