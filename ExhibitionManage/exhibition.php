...
$pageSize = 5;//每一页面内容大小
$res="select * from Exhibition limit 100";
$sql=$conn->query($res);
...
$array=array();
$sql="select * from Exhibition limit $pageSql,$pageSize";
$res=$conn->query($sql);
if ($res) {
...
<div class="allBox">
    ...
    <div class="topWrapper">
        ...
        <div class="searchContent">
            <div class="col-xs-6">
            <form class="form-inline pull-left" role="form" id="search" action="searchexh.php" method="post">
                <div class="form-group">
                    <select class="form-control" name="choice" >
                        <option value="exh_id">展览编号</option>
                        <option value="exh_name">展览名称</option>
                        <option value="mus_id">博物馆编号</option>
                        <option value="mus_name">博物馆名称</option>
                        <option value="exh_info">展览信息</option>
                        <option value="exh_picture">展览图片</option>
                        <option value="exh_time">展览时间</option>
                    </select>
                </div>
            ...
            <button type="button" class="btn btn-info"><a href="addexh.php" class="buttonStyle">添加展览</a></button>
        </div>
        <div class="tableStyle">
            <table style='table-layout:fixed; word-wrap:break-word ; ' class="table table-hover table-bordered" id="showMag">
                ...
                <tr>
                    ...
                    <td><?=$v["exh_name"]?></td>
                    <td><?=$v["mus_id"]?></td>
                    <td><a href="/jk-museum/MuseumManage/showmus.php?mus_name=<?=$v['mus_name']?>"><?=$v["mus_name"]?></a></td>
                    <td>
                        <div style="overflow-y:scroll;height: 100px;">
                        <?=$v["exh_info"]?>
                        </div>
                    </td>
                    <td><img width=120 height=80 src = "<?=$v["exh_picture"]?>" /> </td>
                    <td class="mus_timeTd"><?=$v["exh_time"]?></td>
                    <td>
                        <a href="delexh.php?exh_id=<?=$v['exh_id']?>" class="btn btn-primary">删除</a>
                        <a href="chaexh.php?exh_id=<?=$v['exh_id']?>" class="btn btn-danger">修改</a>
                    </td>
